            ...
            if type(atom) == EnVariable and atom not in variables:  # new variable
                if atom not in new_variables:
                    pred = (
                        ...
                        else clause[clause_pos][0].child
                    )
                    ...
                    new_variables[atom] = [
                        ...
                        list_atoms,
                        len(list_atoms),
                    ]  # clause_pos, atom_pos, atom_list_pos, tree_position, list, list_size
                ...
                if atom not in pos:
                    if type(clause[clause_pos][0]) == EnNot:
                        ...
                        if len(variables_order):
                            ...
                            while (
                                new_variables[variables_order[-1]][2]
                                == new_variables[variables_order[-1]][5]
                            ):
            ...
            if atom_pos == 0:  # it is a predicate
                ...
                if type(predicate) == EnNot:
                    return recursive(
                        ...
                        atom_pos + 1,
                        dict(variables),
                        self.tuples.data[predicate.child],
                        values,
                    )
                else:
                    return recursive(
                ...
                if type(j) == EnAtom:
                    if j not in root:
                        if type(clause[clause_pos][0]) == EnNot:
                            ...
                            if clause_pos + 1 == len(clause):
                                ...
                                return s
                            return recursive(
                                clause_pos + 1, 0, dict(variables), root, v
                            )
                        else:
                            return 0.0
                    else:
                    ...
                    typ = self.bases[
                        str(
                            clause[clause_pos][0].child
                            if type(clause[clause_pos][0]) == EnNot
                            else clause[clause_pos][0]
                        )
                    ][atom_pos - 1]
                    found = 0.0
                ...
                if type(j) == EnAtom:
                    if j not in root:
                        ...
                        if atom_pos + 1 == len(clause[clause_pos]):
                            ...
                            if type(clause[clause_pos][0]) == EnNot:
                                # return 0.0
                                prob = self.literal_to_probability(
                                    clause[clause_pos], variables
                                )
